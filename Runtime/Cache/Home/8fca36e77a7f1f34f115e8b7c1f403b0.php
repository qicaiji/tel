...
<!--    <li class="text-center"><a href="<?php echo U('index/index');?>">首页</a></li>-->
	...
	<table class="table table-bordered table-hover table-striped">
    	<thead>
            <tr>
                <?php if(session('gids') == 1): ?><th class="width-1">ID</th><?php endif; ?>
                <th class="width-2">组名称</th>
                <th class="width-1">状态</th>
                <th class="width-3">权限</th>
                <?php if(session('gids') == 1): ?><th class="width-1">所属企业</th><?php endif; ?>
                <th class="width-2">管理</th>
            </tr>
        </thead>
        <tbody>
            <?php if(is_array($glist)): $i = 0; $__LIST__ = $glist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$one): $mod = ($i % 2 );++$i;?><tr>
                	<?php if(session('gids') == 1): ?><td><?php echo ($one["id"]); ?></td><?php endif; ?>
                    <td><?php echo ($one["title"]); ?></td>
                    <td><?php echo ($one["gstatus"]); ?></td>
                    <td><?php echo ($one["rules_title"]); ?></td>
                    <?php if(session('gids') == 1): ?><td><?php echo ($one["cname"]); ?></td><?php endif; ?>
                    <td>
                        <a href="<?php echo U('edit',array('id'=>$one['id']));?>">编辑</a> | 
                        <a href="<?php echo U('del',array('id'=>$one['id']));?>" onClick="javascript:return del()">删除</a>
                    </td>
